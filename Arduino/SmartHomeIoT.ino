/*
  =========================================================================
  SmartHomeIoT  --  STEP 5 firmware  (sensors + serial protocol only)
  =========================================================================
  PURPOSE OF THIS VERSION:
    This is the FIRST real firmware. It reads every sensor on a timer and
    emits the structured STATUS line that the Python application will later
    parse. It also responds to a few basic serial commands.

    There is intentionally NO control logic yet (no temperature thresholds,
    no light auto mode, no secure mode, no emergency handling). Those are
    added in Steps 6-8. Building the telemetry "contract" first lets us
    verify the Arduino<->PC communication in isolation before piling control
    logic on top.

  WIRING (all confirmed on the bench):
    ...
    D7  - Emergency RGB LED, BLUE leg  (unused by spec)
  ...
  RELAY POLARITY:
    ...
    relay ON; writing HIGH turns it OFF. We wrap this in helper functions
    so the rest of the code can think in plain ON/OFF terms.
  ...
  SERIAL PROTOCOL (the contract with Python):
    Arduino -> PC:
      ...
      Discrete events (added in later steps): EVENT:MOTION / EVENT:EMERGENCY
    PC -> Arduino (one command per line, newline terminated):
      STATUS  -> force an immediate STATUS line
      (the control commands HEAT_ON, COOL_ON, LIGHT_ON, etc. are parsed here
       so the plumbing exists, but in Step 5 they only set the relay state
       directly; full mode/interlock logic arrives in Steps 6-8.)
  =========================================================================
*/
...
// TIMING
// ------------------------------------------------------------------------
// The spec measures every 10 minutes (600000 ms). Waiting that long while
// developing is painful, so we use a fast interval during testing and switch
// to the real value for the demo by flipping this one #define.
//   TEST  : 5000   (5 seconds)
...
// ------------------------------------------------------------------------
// TEMPERATURE CONTROL THRESHOLDS  (exam spec, fixed values)
// ------------------------------------------------------------------------
...
// ------------------------------------------------------------------------
// LIGHT AUTO MODE THRESHOLD  (exam spec)
// ------------------------------------------------------------------------
...
// ------------------------------------------------------------------------
// HOME SECURE MODE TIMING  (exam spec)
// ------------------------------------------------------------------------
...
// LDR CALIBRATION
// ------------------------------------------------------------------------
// During bring-up we saw A1 read higher with MORE light (bright ~958, dim
// ~500). We map the raw 0..1023 ADC value to a 0..100 "illumination percent"
// where bright = high percent. These two anchors can be tuned later; for now
// a straight 0..1023 -> 0..100 map is fine for testing the protocol.
...
float readTemperatureC() {
  ...
  // 2) Simple insertion sort (N is tiny, so this is plenty fast).
  for (int i = 1; i < N; i++) {
    int key = samples[i];
    int j = i - 1;
...
// In Step 5 the relay states are only changed by direct manual commands;
// the mode flags exist but their logic is added later.
...
// ------------------------------------------------------------------------
void applyHeat(bool on)  { digitalWrite(PIN_RELAY_HEAT,  on ? LOW : HIGH); stateHeat  = on; }
void applyCool(bool on)  { digitalWrite(PIN_RELAY_COOL,  on ? LOW : HIGH); stateCool  = on; }
void applyLight(bool on) { digitalWrite(PIN_RELAY_LIGHT, on ? LOW : HIGH); stateLight = on; }

// ------------------------------------------------------------------------
// INTERLOCKED HEAT/COOL SETTERS  (THE SAFETY RULE: never both on)
// ------------------------------------------------------------------------
// The exam states heating and cooling can NEVER run at the same time. Rather
...
void runTemperatureControl(float tempC) {
  ...
  if (stateEmerg)  return;   // emergency overrides everything (handled later)
...
// auto mode; the spec describes secure-mode lighting as its own behavior.
void runSecureMode() {
...
// ------------------------------------------------------------------------
// Clears emergency mode. Per spec, this is the ONLY way out of emergency.
...
// We debounce: only accept a level once it has been stable for the debounce
...
//
// MANUAL/AUTOMATIC RULE (from the exam): a manual heating or cooling command
...
// can never both be on. (Light auto mode has the mirror-image rule, added in
// Step 7; emergency handling is added in Step 8.)
void handleCommand(String cmd) {
  ...
  else if (cmd == "EMERGENCY_OFF") { deactivateEmergency(); return; }
  ...
  if      (cmd == "HEAT_ON")    { stateATC = false; setHeating(true);  }
  else if (cmd == "HEAT_OFF")   { stateATC = false; setHeating(false); }
  else if (cmd == "COOL_ON")    { stateATC = false; setCooling(true);  }
  else if (cmd == "COOL_OFF")   { stateATC = false; setCooling(false); }
  else if (cmd == "LIGHT_ON")   { stateLAuto = false; applyLight(true);  }
  else if (cmd == "LIGHT_OFF")  { stateLAuto = false; applyLight(false); }
  else if (cmd == "ATC_ON")     { stateATC = true;  }
  else if (cmd == "ATC_OFF")    { stateATC = false; }
  else if (cmd == "LAUTO_ON")   { stateLAuto = true;  }
  else if (cmd == "LAUTO_OFF")  { stateLAuto = false; }
  else if (cmd == "SECURE_ON")  { stateSecure = true;  }
  else if (cmd == "SECURE_OFF") { stateSecure = false; }
  else {
    // Unknown command: report it but do not act. (EMERGENCY_OFF and the
    // emergency rules are handled in Step 8.)
